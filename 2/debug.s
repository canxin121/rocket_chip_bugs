...
_user_code:
    ...
    li x31, 0x4
    li t0, 0x1234ABCD
    sw t0, 0(x31) # Address 0x4: Spike triggers Store/AMO access fault, Rocket triggers Store/AMO access fault (both are access errors)
    lw t0, 0(x31) # Address 0x4: Spike triggers Load access fault, Rocket triggers Load access fault (both are access errors)
    amoxor.w.aqrl x3, x9, (x31) # Address 0x4: Spike triggers Store/AMO access fault, Rocket triggers Store/AMO access fault (both are access errors)
    DUMP_ALL_REGS framework_temp_save_area

    li x31, 0x5
    li t0, 0x1234ABCD
    sw t0, 0(x31) # Address 0x5: Spike triggers Store/AMO address misaligned, Rocket triggers Store/AMO access fault (different exception types!)
    lw t0, 0(x31) # Address 0x5: Spike triggers Load address misaligned, Rocket triggers Load access fault (different exception types!)
    amoxor.w.aqrl x3, x9, (x31) # Address 0x5: Spike triggers Store/AMO address misaligned, Rocket triggers Store/AMO access fault (different exception types!)
    DUMP_ALL_REGS framework_temp_save_area

    li x31, 0xFFFFFFE1
    li t0, 0x1234ABCD
    sw t0, 0(x31) # Address 0xFFFFFFE1: Spike triggers Store/AMO address misaligned, Rocket triggers Store/AMO access fault (different exception types!)
    lw t0, 0(x31) # Address 0xFFFFFFE1: Spike triggers Load address misaligned, Rocket triggers Load access fault (different exception types!)
    amoxor.w.aqrl x3, x9, (x31) # Address 0xFFFFFFE1: Spike triggers Store/AMO address misaligned, Rocket triggers Store/AMO access fault (different exception types!)
    DUMP_ALL_REGS framework_temp_save_area

    li x31, 0xFFFFFFEC
    li t0, 0x1234ABCD
    sw t0, 0(x31) # Address 0xFFFFFFEC: Spike executes normally (no exception), Rocket triggers Store/AMO access fault (completely different behavior!)
    lw t0, 0(x31) # Address 0xFFFFFFEC: Spike executes normally (no exception), Rocket triggers Load access fault (completely different behavior!)
    amoxor.w.aqrl x3, x9, (x31) # Address 0xFFFFFFEC: Spike executes normally (no exception), Rocket triggers Store/AMO access fault (completely different behavior!)
    DUMP_ALL_REGS framework_temp_save_area # Result: Rocket keeps x3=0xDEADBEEF (exception prevents execution), Spike successfully executes to get x3=0x1234ABCD
